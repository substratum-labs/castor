...
class TestCreateCapabilities:
    def test_create_from_specs(self, mgr):
        caps = mgr.create_capabilities({"network_read": 100.0, "disk_delete": 10.0})
        assert len(caps) == 2
...
class TestDeduct:
    ...
    def test_deduct_exceeds_budget(self, mgr):
        ...
        with pytest.raises(CapabilityExhaustedError) as exc_info:
...
class TestDelegate:
    def test_delegate_subset(self, mgr):
        parent = mgr.create_capabilities({"api": 100.0, "disk": 50.0})
    ...
    def test_delegate_exceeds_parent(self, mgr):
        parent = mgr.create_capabilities({"api": 10.0})
...
class TestReclaim:
    ...
    def test_full_lifecycle(self, mgr):
        ...
        parent = mgr.create_capabilities({"net": 100.0, "disk": 50.0})
