"""Integration tests for Castor CLI (subprocess-based)."""
...
def run_cli(*args: str, db_path: Path) -> subprocess.CompletedProcess[str]:
    ...
    cmd = [sys.executable, "-m", "castor.cli", "--db", str(db_path), *args]
...
class TestCLIPs:
    def test_ps_empty(self, temp_db):
        result = run_cli("ps", db_path=temp_db)
...
class TestCLIInspect:
    ...
    def test_inspect_missing_pid(self, temp_db):
        result = run_cli("inspect", "ghost-999", db_path=temp_db)
