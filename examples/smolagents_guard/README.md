...
These are the same gaps behind [OpenClaw's security incidents](https://github.com/openclaw/openclaw/issues/2023) (agent sending messages to wrong recipients, executing destructive operations without approval) and [NCC Group's smolagents audit](https://www.nccgroup.com/research/autonomous-ai-agents-a-hidden-risk-in-insecure-smolagents-codeagent-usage/) (code execution without human review).
...
```bash
# Level 1: budget + HITL
uv run python examples/smolagents_guard/demo.py

# Level 2: crash recovery + HITL suspend/resume
uv run python examples/smolagents_guard/demo_deep.py

# Tests
uv run pytest examples/smolagents_guard/ -v
```
...
| Lodge context management | smolagents manages its own context window | Use full Castor for Lodge |
| Multi-agent spawning | Requires Castor's `SyscallProxy` async pipeline | Use full Castor for sub-agents |

These limitations exist because smolagents owns its agent loop. Castor's guard layer hooks into the loop but doesn't replace it. For full control, use Castor's native `AgentRunner` + `SyscallProxy` (see `examples/openclaw_agent/`).
