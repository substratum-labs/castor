...
`AgentRequest` and `OperatorRequest` have distinct operation types, and `send()` returns an `AisaResponse` with a correlated request ID and outcome. The protocol's individual payload and outcome schemas remain dictionaries; `castord` validates their semantic content.
