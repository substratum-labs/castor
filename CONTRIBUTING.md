...
Castor is a microkernel with four subsystems plus a standard library:

| Module | Purpose |
|--------|---------|
| `gate/` | Tool registry and Pydantic validation (SyscallGate) |
| `scheduler/` | Checkpoint/replay scheduler, HITL, persistence |
| `mmu/` | Context window memory management (MMU) |
| `capability/` | Budget tracking and delegation |
| `lib/` | Agent-facing primitives (`tool`, `chat`, `spawn`) and patterns (`react`, `parallel`, `supervisor`) |
| `cli/` | `castor run`, `castor ps`, `castor inspect`, HITL commands |
| `mcp/` | Expose `@castor_tool` as MCP tools via FastMCP |
